...
import { invalidateUserPasswordResetSessions } from '$lib/server/auth/passwordReset'
...
const verifyCode = async (event: RequestEvent) => {
...
const resendEmail = async (event: RequestEvent) => {
  if (event.locals.session === null || event.locals.user === null) {
    return fail(401, {
      resend: {
        message: 'Not authenticated'
